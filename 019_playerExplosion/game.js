...
function enterFrameByCore() {
    for(var i=0; i<4; i++){
        if(enemyArray[i].visible===true && player.intersect(enemyArray[i]) && player.visible===true){
            player.visible = false;
            core.rootScene.addChild(createExplosion(player.x+32,player.y+32));
        }
        for(var j=0; j<3; j++){
            if(shotArray[j].intersect(enemyArray[i]) &&
                shotArray[j].visible===true && enemyArray[i].visible===true){
                core.rootScene.addChild(createExplosion(enemyArray[i].x+32,enemyArray[i].y+32));
